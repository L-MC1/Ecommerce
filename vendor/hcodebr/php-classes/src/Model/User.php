...
class User extends Model {
	...
	public static function login($login, $password){
		...
		if (count($results) === 0) {
			throw new \Exception("Uusário inexistente ou senha inválida.");			
		}
		$data = $results[0];
	...
	public static function getForgot($email, $inadmin = true){
		...
		if(count($results) === 0){
			throw new \Execption("Não foi possível recuperar a senha.");
		} else {
			$data = $results[0];
	...
	public static function validForgotDecrypt($code){		
		...
		if(count($results) === 0){
			throw new \Exception("Nao foi possível recuperar sua senha");			
		} else{
			return $results[0];
		}
	}
